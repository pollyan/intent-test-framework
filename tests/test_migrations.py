    ...
    def test_migration_idempotency(self, app):
        ...
        with app.app_context():
            ...
            retrieved_var = ExecutionVariable.query.filter_by(
                execution_id="test-exec", variable_name="test_var"
            ).first()
...
class TestMigrationPerformance:
    ...
    def test_large_data_migration_performance(self, app):
        ...
        with app.app_context():
            ...
            # 测试索引查询性能
            start_time = time.time()
            result = ExecutionVariable.query.filter_by(execution_id="perf-test-0").all()
    ...
    def test_sql_rollback_script(self):
        ...
        try:
            ...
            rollback_file = os.path.join(
                ...
                "migrations",
                "sql",
                "001_rollback_variable_tables.sql",
            )
            ...
            # 验证表已删除
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%variable%'"
            )
