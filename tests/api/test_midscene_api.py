...
class TestMidSceneExecutionAPI:
    ...
    def test_should_receive_execution_result_success(self, api_client, create_test_testcase, 
        ...
        testcase = create_test_testcase({
            ...
            'steps': [
                ...
                {'action': 'ai_input', 'params': {'text': 'test', 'locate': '搜索框'}}
            ]
        })
            ...
            'step_results': [
                {
                    ...
                    'result_data': {'url': 'https://example.com', 'title': 'Example Site'}
                },
                {
                    'step_index': 1,
                    'action': 'ai_input',
...
class TestMidSceneExecutionStartAPI:
    ...
    def test_should_receive_execution_start_notification(self, api_client, create_test_testcase, 
                                                        create_test_execution, assert_api_response):
        """测试接收执行开始通知"""
        testcase = create_test_testcase({
    ...
    def test_should_validate_execution_start_data(self, api_client):
        """测试验证执行开始通知数据"""
        # 缺少必需字段
        response = api_client.post('/api/midscene/execution-start', json={
...
class TestMidSceneServiceIntegration:
    ...
    def test_should_handle_concurrent_executions(self, api_client, create_test_testcase, 
        ...
        for i in range(3):
            testcase = create_test_testcase({
                'name': f'并发测试用例{i}',
                'steps': [{'action': 'goto', 'params': {'url': f'https://example{i}.com'}}]
            })
...
class TestMidSceneErrorHandling:
    ...
    def test_should_handle_timeout_scenarios(self, api_client, create_test_testcase, 
                                            create_test_execution, assert_api_response):
        """测试处理超时场景"""
        testcase = create_test_testcase({
            'name': '超时测试用例',
            'steps': [{'action': 'ai_wait_for', 'params': {'condition': '页面加载完成'}}]
        })
    ...
    def test_should_handle_network_errors(self, api_client, create_test_testcase, 
        ...
        testcase = create_test_testcase({
            'name': '网络错误测试用例',
            'steps': [{'action': 'goto', 'params': {'url': 'https://nonexistent-domain.example'}}]
        })
    ...
    def test_should_handle_duplicate_results(self, api_client, create_test_testcase, 
        ...
        # 发送第一次执行结果
        response1 = api_client.post('/api/midscene/execution-result', json=execution_result)
        assert response1.status_code == 200
        
        # 发送重复的执行结果
        response2 = api_client.post('/api/midscene/execution-result', json=execution_result)
