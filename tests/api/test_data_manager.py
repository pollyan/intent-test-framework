...
class APITestDataManager:
    ...
    def create_execution(self, data: Optional[Dict[str, Any]] = None) -> 'ExecutionProxy':
        """创建执行记录"""
    ...
    def create_step_execution(self, execution_id: str, data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """创建步骤执行记录"""
        default_data = {
            'step_index': 0,
            'action': 'goto',
        ...
        step_execution = StepExecution(
            ...
            status=default_data['status'],
            start_time=default_data['start_time'],
            end_time=default_data.get('end_time', default_data['start_time'] + timedelta(milliseconds=default_data['duration'])),
    ...
    def create_bulk_testcases(self, count: int, name_prefix: str = '批量测试用例') -> List[TestCaseProxy]:
        """批量创建测试用例"""
        testcases = []
    ...
    def create_execution_with_steps(self, testcase_id: int, step_count: int = 3) -> Dict[str, Any]:
        """创建包含多个步骤的完整执行记录"""
        # 创建执行记录
        ...
        for i in range(step_count):
            ...
            step_execution = self.create_step_execution(execution['execution_id'], step_data)
    ...
    def create_test_scenario(self, scenario_name: str) -> Dict[str, Any]:
        ...
        testcase = self.create_testcase({
            ...
            'steps': [
                ...
                {'action': 'ai_input', 'params': {'text': 'test', 'locate': '输入框'}},
                {'action': 'ai_tap', 'params': {'locate': '提交按钮'}},
                {'action': 'ai_assert', 'params': {'condition': '页面显示成功消息'}}
            ]
        })
    ...
    def cleanup_all(self):
        ...
        try:
            ...
            for execution in self.created_items['executions']:
                exec_record = self.db_session.query(ExecutionHistory).get(execution['id'])
                if exec_record:
...
class TestDataPresets:
    ...
    @staticmethod
    def get_minimal_testcase() -> Dict[str, Any]:
        """最小测试用例数据"""
        return {
            'name': '最小测试用例',
    ...
    @staticmethod
    def get_complex_testcase() -> Dict[str, Any]:
        """复杂测试用例数据"""
        return {
            'name': '复杂UI自动化测试',
            'description': '包含多种操作的复杂测试用例',
            'category': 'UI自动化',
            'priority': 1,
            'tags': ['ui', 'complex', 'automation'],
            'steps': [
                {
                    'action': 'goto',
                    'params': {'url': 'https://complex-app.example.com'},
                    'description': '访问复杂应用'
                },
                {
                    'action': 'ai_input',
                    'params': {'text': 'admin', 'locate': 'input[name="username"]'},
                    'description': '输入用户名'
                },
                {
                    'action': 'ai_input',
                    'params': {'text': 'password123', 'locate': 'input[name="password"]'},
                    'description': '输入密码'
                },
                {
                    'action': 'ai_tap',
                    'params': {'locate': 'button[type="submit"]'},
                    'description': '点击登录按钮'
                },
                {
                    'action': 'ai_wait_for',
                ...
                },
                {
                    ...
                    'params': {
                        'query': '提取用户信息',
                        'dataDemand': '{"username": "string", "role": "string"}'
                    },
                    'description': '提取用户信息',
                ...
                {
                    'action': 'ai_hover',
                    'params': {'locate': '.user-menu'},
                    'description': '悬停用户菜单'
                },
                {
                    'action': 'ai_tap',
                    'params': {'locate': 'a[href="/logout"]'},
                    'description': '点击退出按钮'
                }
            ]
        }
    ...
    @staticmethod
    def get_error_testcase() -> Dict[str, Any]:
        """会导致错误的测试用例数据"""
        return {
            'name': '错误测试用例',
            'description': '故意包含错误的测试用例',
            'steps': [
                {
                    'action': 'goto',
                    'params': {'url': 'https://nonexistent-domain-12345.com'},
                    'description': '访问不存在的网站'
                },
                {
                    'action': 'ai_tap',
                    'params': {'locate': '绝对不存在的元素'},
    ...
    @staticmethod
    def get_performance_testcase() -> Dict[str, Any]:
        """性能测试用例数据"""
        return {
            'name': '性能测试用例',
            'description': '用于性能测试的测试用例',
            'category': '性能测试',
            'steps': [
                {
                    'action': 'goto',
                    'params': {'url': 'https://fast-site.example.com'},
                    'description': '访问快速网站'
                },
                {
                    'action': 'ai_wait_for',
                    'params': {'condition': '页面完全加载', 'timeout': 5000},
                    'description': '等待页面加载（5秒超时）'
                },
                {
                    'action': 'ai_assert',
                    'params': {'condition': '页面加载时间小于3秒'},
                    'description': '验证性能要求'
                }
            ]
        }
    ...
    @staticmethod
    def get_invalid_testcase_data() -> List[Dict[str, Any]]:
        """各种无效测试用例数据"""
        return [
            {
                'name': '',  # 空名称
            ...
            {
                'name': '无效步骤测试',
                'steps': [{'invalid_field': 'value'}]  # 无效步骤格式
            },
            {
                'name': '无效动作测试',
                'steps': [{'action': 'invalid_action', 'params': {}}]  # 无效动作
            }
        ]