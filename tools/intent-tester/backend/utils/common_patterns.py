...
def paginated_query(
    query_builder: Callable, item_transformer: Optional[Callable] = None
):
    """
    分页查询装饰器
    统一处理分页逻辑和响应格式
...
def validate_resource_exists(
    model_class: Type, id_param: str = "id", error_message: Optional[str] = None
):
    """
    验证资源存在装饰器
    """
...
def require_json_data(
    required_fields: Optional[list] = None, optional_fields: Optional[list] = None
):
    """
    要求JSON数据装饰器
    """
...
class APIResponseHelper:
    ...
    @staticmethod
    def paginated(
        items: list, page: int, size: int, total: int, message: str = "获取成功"
    ) -> Dict[str, Any]:
        """分页响应"""
        return {
            "code": 200,
