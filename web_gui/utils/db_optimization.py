...
def create_database_indexes(db):
    ...
    indexes = [
        # 测试用例表索引
        (
            "idx_testcases_is_active",
            "CREATE INDEX IF NOT EXISTS idx_testcases_is_active ON test_cases(is_active)",
        ),
        (
            "idx_testcases_category",
            "CREATE INDEX IF NOT EXISTS idx_testcases_category ON test_cases(category)",
        ),
        (
            "idx_testcases_updated_at",
            "CREATE INDEX IF NOT EXISTS idx_testcases_updated_at ON test_cases(updated_at DESC)",
        ),
        (
            "idx_testcases_name",
            "CREATE INDEX IF NOT EXISTS idx_testcases_name ON test_cases(name)",
        ),
        # 执行历史表索引
        (
            "idx_execution_history_test_case_id",
        ...
        for table in tables:
            ...
            if table in ["test_cases", "execution_history"]:
                ...
                try:
                    ...
                    stats[table]["latest_update"] = (
                        recent_time.isoformat() if recent_time else None
                    )
                except:
...
def cleanup_old_executions(db, days_to_keep: int = 30):
    ...
    try:
        ...
        step_result = db.session.execute(
            text(step_delete_sql), {"cutoff_date": cutoff_date}
        )
        history_result = db.session.execute(
            text(history_delete_sql), {"cutoff_date": cutoff_date}
        )

        db.session.commit()

        logger.info(
            f"🧹 清理完成: 删除了 {history_result.rowcount} 条执行记录和 {step_result.rowcount} 条步骤记录"
        )
        return {
