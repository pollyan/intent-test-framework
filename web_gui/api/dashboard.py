...
def get_recent_activities():
    ...
    # 最近的执行记录
    recent_executions = (
        db.session.query(ExecutionHistory, TestCase.name.label("testcase_name"))
        .join(TestCase)
        .order_by(ExecutionHistory.start_time.desc())
        .limit(limit)
        .all()
    )
...
def get_execution_chart():
    ...
    if days <= 1:
        ...
        hourly_stats = (
            ...
            .group_by(func.date_trunc("hour", ExecutionHistory.start_time))
            .order_by("hour")
            .all()
        )

        chart_data = [
            {"time": hour.strftime("%H:00"), "count": count}
        ...
        daily_stats = (
            ...
            .filter(ExecutionHistory.start_time >= start_date)
            .group_by(func.date(ExecutionHistory.start_time))
            .order_by("date")
            .all()
        )
        ...
        for date, count in daily_stats:
            ...
            chart_data.append({"time": time_str, "count": count})
...
def get_top_testcases():
    ...
    subquery = (
        ...
        .filter(TestCase.is_active == True, ExecutionHistory.start_time >= start_date)
        .group_by(TestCase.id, TestCase.name, TestCase.category)
    ...
    top_testcases = db.session.query(
        subquery.c.id, subquery.c.name, subquery.c.category, subquery.c.execution_count
    ).all()
    ...
    for tc_id, name, category, exec_count in top_testcases:
        # 单独查询成功次数来避免SQLite问题
        success_count = (
            db.session.query(func.count(ExecutionHistory.id))
            .filter(
...
def get_failure_analysis():
    ...
    failure_reasons = (
        ...
        .group_by(ExecutionHistory.error_message)
        .order_by(desc("count"))
        .limit(10)
        .all()
    )
    ...
    failed_steps = (
        db.session.query(
            StepExecution.step_description, func.count(StepExecution.id).label("count")
        )
        .join(ExecutionHistory)
        .filter(
            and_(
        ...
        .group_by(StepExecution.step_description)
        .order_by(desc("count"))
        .limit(10)
        .all()
    )
    ...
    failure_prone_testcases = (
        ...
        .join(ExecutionHistory)
        .filter(
            and_(TestCase.is_active == True, ExecutionHistory.start_time >= start_date)
        )
        .group_by(TestCase.id, TestCase.name)
        .having(func.count(ExecutionHistory.id) >= 5)  # 至少5次执行
        .all()
    )
...
def get_system_health():
    ...
    # 计算成功率
    success_rate = (
        (recent_success / recent_executions * 100) if recent_executions > 0 else 100
    )
