...
def get_db_status():
    ...
    try:
            ...
            for table_name in tables_to_check:
                try:
                    ...
                    db_info["errors"].append(
                        f"表 {table_name} 检查失败: {str(table_error)}"
                    )
        ...
        try:
            executions = (
                ExecutionHistory.query.order_by(ExecutionHistory.created_at.desc())
                .limit(5)
                .all()
            )
                ...
                recent_executions.append(
                    {
                        ...
                        "status": exec.status,
                        "created_at": (
                            exec.created_at.strftime("%Y-%m-%dT%H:%M:%S.%fZ")
                            if exec.created_at
                            else None
                        ),
                    }
        ...
        return jsonify({"code": 500, "message": f"数据库状态检查失败: {str(e)}"}), 500
...
def create_test_data():
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({"code": 500, "message": f"创建测试数据失败: {str(e)}"}), 500
