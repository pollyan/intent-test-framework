...
def midscene_execution_result():
    ...
    try:
        ...
        if not request.is_json:
            return jsonify({
                'code': 415,
                'message': '不支持的内容类型，请使用application/json'
            }), 415
        ...
        for i, step_data in enumerate(steps_data):
            ...
            if step_data.get('start_time'):
                step_start_time = datetime.fromisoformat(step_data['start_time'].replace('Z', '+00:00'))
            if step_data.get('end_time'):
                step_end_time = datetime.fromisoformat(step_data['end_time'].replace('Z', '+00:00'))
            ...
            step_execution = StepExecution(
                ...
                step_description=step_data.get('description', f"{step_data.get('action', 'unknown')} 步骤"),
    ...
    try:
        ...
        if execution:
            ...
            if data.get('start_time'):
                try:
                    execution.start_time = datetime.fromisoformat(data['start_time'].replace('Z', '+00:00'))
                except:
