...
def downgrade():
    ...
    try:
        ...
        op.drop_constraint(
            "uk_execution_variable_name", "execution_variables", type_="unique"
        )
        print("✓ 删除约束成功")
    except Exception as e:
        print(f"删除约束时出错: {e}")
...
def _create_indexes_safe():
    ...
    indexes = [
        ...
        (
            "idx_reference_execution_step",
            "variable_references",
            ["execution_id", "step_index"],
        ),
        (
            "idx_reference_variable",
            "variable_references",
            ["execution_id", "variable_name"],
        ),
        (
            "idx_reference_status",
            "variable_references",
            ["execution_id", "resolution_status"],
        ),
    ]
