...
def test_ui_methods():
    ...
    with app.app_context():
        try:
            ...
            assert result1[
                "success"
            ], f"aiQuery测试失败: {result1.get('error_message')}"
            ...
            assert result3[
                "success"
            ], f"aiInput测试失败: {result3.get('error_message')}"
            print("✓ aiInput: 变量引用输入成功")

            # 测试4: aiString方法
            step4 = {
                "action": "aiString",
                "params": {"query": "获取页面标题或主要文本内容"},
            ...
            assert result4[
                "success"
            ], f"aiString测试失败: {result4.get('error_message')}"
            ...
            assert result6[
                "success"
            ], f"aiAssert测试失败: {result6.get('error_message')}"
            ...
            assert result7[
                "success"
            ], f"aiWaitFor测试失败: {result7.get('error_message')}"
            ...
            step8 = {
                ...
                "params": {
                    "script": 'return {url: window.location.href, title: document.title, productName: "'
                    + "${product_data.商品名称}"
                    + '", timestamp: Date.now()};'
                },
            ...
            assert result8[
                "success"
            ], f"evaluateJavaScript测试失败: {result8.get('error_message')}"
