    ...
    def verify_migration(self) -> bool:
        ...
        try:
            ...
            for table_name in tables:
                ...
                # 检查目标数据库记录数
                with self.target_engine.connect() as target_conn:
                    target_result = target_conn.execute(
                        text(f"SELECT COUNT(*) FROM {table_name}")
                    )
                ...
                if source_count == target_count:
                    self.log(
                        f"✅ 表 {table_name}: {source_count} -> {target_count} 记录"
                    )
                else:
                    self.log(
                        f"⚠️  表 {table_name}: {source_count} -> {target_count} 记录不匹配",
                        "WARNING",
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="SQLite到Supabase PostgreSQL数据迁移工具"
    )
