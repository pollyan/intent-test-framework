...
def test_data_flow_integration():
    ...
    with app.app_context():
        try:
            ...
            step2 = {
                "action": "aiQuery",
                "params": {"dataDemand": "{name: string, price: number, id: string}"},
            ...
            for var in available_vars:
                print(
                    f"  - {var['name']}: {var['data_type']} = {str(var['value'])[:50]}..."
                )
            ...
            for expr in test_expressions:
                ...
                for result in validation_results:
                    ...
                    print(
                        f"    {status} {result['reference_path']}: {result.get('error_message', 'OK')}"
                    )
