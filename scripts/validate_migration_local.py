...
def validate_migration():
    ...
    with app.app_context():
        try:
            ...
            # 验证字段是否存在
            ev_columns = [
                col["name"] for col in inspector.get_columns("execution_variables")
            ]
            required_ev_columns = [
                "id",
                "execution_id",
            ...
            vr_columns = [
                col["name"] for col in inspector.get_columns("variable_references")
            ]
            ...
            # 验证索引是否存在
            ev_indexes = [
                idx["name"] for idx in inspector.get_indexes("execution_variables")
            ]
            logger.info(f"execution_variables索引: {ev_indexes}")

            vr_indexes = [
                idx["name"] for idx in inspector.get_indexes("variable_references")
            ]
...
def test_crud_operations():
    ...
    try:
        ...
        retrieved_var = ExecutionVariable.query.filter_by(
            execution_id="test-exec-001", variable_name="test_variable"
        ).first()
