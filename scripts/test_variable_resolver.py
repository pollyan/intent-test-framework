...
def test_variable_resolver():
    ...
    with app.app_context():
        try:
            ...
            assert (
                resolved_params["text"] == "用户名是：张三"
            ), f"文本解析错误: {resolved_params['text']}"
            assert (
                resolved_params["url"] == "https://example.com/user/张三"
            ), f"URL解析错误: {resolved_params['url']}"
            print("✓ 简单变量引用解析成功")
            ...
            assert (
                "${product_info.nonexistent_property}" in resolved_error["invalid_prop"]
            )
            ...
            # 检查产品信息变量的属性
            product_var = next(
                var for var in available_vars if var["name"] == "product_info"
            )
            ...
            # 数组索引格式调整
            has_tags_array = any(
                "tags.[0]" in prop or "tags[0]" in prop
                for prop in product_var["properties"]
            )
            assert (
                has_tags_array
            ), f"未找到tags数组属性，实际属性: {product_var['properties']}"
            ...
            assert (
                len(validation_results) == 4
            ), f"验证结果数量不正确: {len(validation_results)}"
