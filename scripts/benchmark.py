...
class TestBenchmark:
    ...
    def run_benchmark(self, test_path: str = "tests/api", parallel: bool = False) -> Dict[str, Any]:
        """运行性能基准测试"""
        ...
        cmd = [
            ...
            "-v",
            "--benchmark-json=benchmark.json" if self._has_benchmark_plugin() else "--json-report",
        ]
    ...
    def analyze_performance(self, benchmark_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        analysis["summary"] = {
            ...
            "slow_test_count": len([t for t in slow_tests if t["duration"] > self.threshold_slow]),
            "fast_test_count": len([t for t in slow_tests if t["duration"] < self.threshold_fast]),
        }
    ...
    def _generate_recommendations(self, summary: Dict[str, Any]) -> List[str]:
        ...
        if slow_count > total_tests * 0.1:  # 超过10%的测试是慢速的
            recommendations.append("🐌 发现过多慢速测试，建议使用@pytest.mark.slow标记并在快速测试中排除")
            recommendations.append("📊 考虑将慢速测试拆分为更小的测试单元")
        
        # 并行化建议
        if total_tests > 10 and not recommendations:
            recommendations.append("🚀 测试数量较多，建议启用并行测试: pip install pytest-xdist")
...
def main():
    parser = argparse.ArgumentParser(description="Intent Test Framework 性能基准测试工具")
    parser.add_argument("--path", default="tests/api", help="测试路径 (默认: tests/api)")
